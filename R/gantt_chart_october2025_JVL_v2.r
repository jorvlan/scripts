...
tasks <- data.frame(
  label     = c("Data preprocessing", "Statistical modeling", "Attend weekly group meeting", "Organize workshop", "Present findings to department", "Article writing"),
  start_wk  = c(1, 4, 1, 8, 12, 8),   # starting week
  duration  = c(3, 8, 12, 1, 1, 5),   # duration in weeks
  repeats   = c(1, 1, 1, 1, 1, 1)    # number of repeats
)
...
# ---- Plot ----
gantt_sydney <- ggplot(tasks_expanded, aes(x = start_date, xend = end_date,
                           y = label, yend = label)) +
  geom_segment(size = 7, lineend = "butt", color = "#3CBB75FF") +  # thick bars
  ...
  labs(title = "Timeline - research visit", x = "By week", y = "") +
