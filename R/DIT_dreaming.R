...
p <- qplot(x, y, data=tmp, xlab = 'Apical state', ylab = "Consciousness") +
  geom_abline(intercept = 0, slope = 0.328, color = 'dodgerblue', size = 2, linetype = 1, alpha = .8) +
  geom_abline(intercept = .3, slope = -.3, color = 'darkorange', size = 2, linetype = 1, alpha = .8)
...
p <- p + annotate("text", label = "Apical drive - cellular mechanism of dreaming (Aru et al., 2020; in press;
                  Neuroscience & Biobehavioral Reviews)", x = .5, y = .28, size = 4)
